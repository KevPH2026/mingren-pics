...
export default function PaywallModal() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (data.isNewUser) {
        // 新用户：密码已填好，直接用 tempToken 注册
        setIsNewUser(true);
        setTempToken(data.tempToken);
        // 直接调用 set-password 完成注册
        setSettingPassword(true);
        try {
          const setResp = await fetch('/api/auth/set-password', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email, password, tempToken: data.tempToken }),
          });
          const setData = await setResp.json();
          if (!setResp.ok) {
            setError(setData.error || '注册失败');
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-sm bg-white comic-border animate-bounce-in">
          ...
          <div className="p-5 flex flex-col gap-4">
            ...
            {/* 忘记密码 → 重发验证码 → 走 reset-password 流程 */}
            <button
              onClick={async () => {
                setPasswordError('');
                setLoggingIn(true);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-sm bg-white comic-border animate-bounce-in">
          ...
          <div className="p-5 flex flex-col gap-4">
            ...
            {/* 已填信息确认 */}
            <div className="bg-gray-50 rounded-lg p-2.5 flex flex-col gap-1">
              <div className="flex items-center gap-2 text-xs">
                <span className="text-black/40 shrink-0">📧</span>
                <span className="font-bold text-black/60 truncate">{maskEmail(email)}</span>
                <span className="text-green-500 font-black ml-auto">✓</span>
              </div>
              <div className="flex items-center gap-2 text-xs">
                <span className="text-black/40 shrink-0">🔑</span>
                <span className="font-bold text-black/60">密码已设置</span>
                <span className="text-green-500 font-black ml-auto">✓</span>
              </div>
            </div>
  ...
  // ============================
  // Render: email step (default) — 邮箱 + 密码 + 发送验证码
  // ============================
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowPaywall(false)} />
      ...
      <div className="relative w-full max-w-sm bg-white comic-border animate-bounce-in max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 flex flex-col gap-3">
          ...
          {/* Step 1: 邮箱 */}
          <div>
            <label className="text-[10px] font-black text-black/40 uppercase tracking-wider mb-1 block">Step 1 · 邮箱</label>
            <input
              type="email"
              value={email}
          ...
          {/* Step 2: 密码 */}
          <div>
            <label className="text-[10px] font-black text-black/40 uppercase tracking-wider mb-1 block">Step 2 · 密码</label>
            <input
              type="password"
              value={password}
            ...
            <input
              ...
              onChange={(e) => { setConfirmPassword(e.target.value); setPasswordError(''); setError(''); }}
              placeholder="🔑 再次输入密码"
              className="w-full py-3 px-3 border-2 border-black text-sm font-bold focus:outline-none focus:border-[#8b5cf6] mt-2"
            />
            {/* real-time validation hints */}
            <div className="flex flex-col gap-0.5 mt-1.5 px-1">
              <span className={password.length >= 6 ? 'text-green-500' : 'text-black/25'}>
                {password.length >= 6 ? '✅' : '⬜'} <span className="text-[10px] font-bold">密码至少6位</span>
              </span>
              <span className={password === confirmPassword && confirmPassword.length > 0 ? 'text-green-500' : 'text-black/25'}>
                {password === confirmPassword && confirmPassword.length > 0 ? '✅' : '⬜'} <span className="text-[10px] font-bold">两次密码一致</span>
              </span>
            </div>
          </div>
          ...
          {/* Step 3: 发送验证码 */}
          <div>
            <label className="text-[10px] font-black text-black/40 uppercase tracking-wider mb-1 block">Step 3 · 验证码</label>
            <div className="flex gap-2">
              <input
                type="text"
                value={code}
                ...
                placeholder="🔢 输入6位验证码"
                maxLength={6}
                className="flex-1 py-3 px-3 border-2 border-black text-sm font-bold text-center tracking-[6px] focus:outline-none focus:border-[#8b5cf6]"
              />
              <button
                onClick={handleSendCode}
                disabled={sending || !email || password.length < 6 || password !== confirmPassword}
              ...
              >
                {sent ? (countdown > 0 ? `${countdown}s` : '重发') : '发送'}
              </button>
            </div>
          </div>
          ...
          <button
            onClick={handleVerify}
            disabled={code.length < 6 || !sent || verifying}
            className="w-full py-3.5 bg-[#8b5cf6] text-white border-2 border-black font-black text-base comic-shadow-sm hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all disabled:opacity-40 mt-1"
          >
            {verifying ? '⏳ 注册中...' : '🚀 完成注册'}
          </button>
          ...
          <div className="flex items-center justify-between">
            <button
              ...
              className="text-[#8b5cf6] text-xs font-bold py-1 hover:underline transition-colors"
            >
              已有账号？密码登录 →
            </button>
            <button
              onClick={() => setShowPaywall(false)}
              className="text-black/30 text-xs font-bold py-1 hover:text-[#e00] transition-colors"
            >
              下次再说
            </button>
          </div>
